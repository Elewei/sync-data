...
for i in range(len(data)):
	...
	for suning_productCode, taobao_id in data[i].items():
		...
		size = 800, 800
		rgb_taobao_image = taobao_image.convert('RGBA')
		color_0 = rgb_taobao_image.getpixel((0, 0))
		for h in range(800):
			for l in range(800):
				dot = (l, h)
				color_1 = rgb_taobao_image.getpixel(dot)
				if color_1 == color_0:
					color_1 = color_1[:-1] + (0,)
					rgb_taobao_image.putpixel(dot, color_1)
