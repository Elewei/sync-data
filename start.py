...
for i in range(len(data)):
	...
	for suning_productCode, taobao_id in data[i].items():
		...
		for list in skuList:
			...
			rel_price = 0.5 * float(skuMap[pvs]['price']) 
			# 淘宝项目中添加 颜色
			...
			# 淘宝项目中添加 库存
			taobao_item.append(skuMap[pvs]['stock'])
			# 淘宝项目中添加 折扣
		...
		# 获取苏宁商品价格 
		#suning_price = suning.get_suning_product_price(browser, suning_products['color'][0], suning_products['size'][0])

		# 获取苏宁商品库存 
		#suning_kucun = suning.get_suning_product_kucun()
